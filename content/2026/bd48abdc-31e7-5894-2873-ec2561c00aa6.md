...
GitHub経由で提供しているNS版Emacsのインラインパッチについて、ビルド済みパッケージの頒布物を整理しました。現在は(手元に)動作確認する手段がない古いパッケージを削除して、今後はEmacs 27.2以降のバージョンにリソースを集中します。  

[Pre-built distribution package](https://github.com/takaxp/ns-inline-patch?tab=readme-ov-file#pre-built-distribution-package)  

主な変更点：  

    GitHub Actions でビルド可能な macOS でインストーラを作成  
    ...
    Emacs 29.4以降のインストーラに TREE\_SITTER を同梱  
        tree-sitter@0.26がリリースされた影響でビルドが失敗するのをパッチで回避してあります。  
            Emacs 29.4/30.2に適用
            Master branch (HEAD)は本家で対応済み
    Emacs 29.4以降のインストーラは NativeComp の有・無の両方を提供  
        ...
        Master branch (HEAD)は、NativeComp有りのみのインストーラを提供  
            HEAD を追う人であれば、NativeComp無しは不要な選択肢です（よね？）
